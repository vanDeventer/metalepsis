...
func (ua *UnitAsset) updateDB(w http.ResponseWriter, r *http.Request) {
	...
	case "POST", "PUT":
		...
		// Split the content type at the first semicolon (if it exists)
		if strings.Contains(headerContentTtype, ";") {
			headerContentTtype = strings.Split(headerContentTtype, ";")[0]
		}
		// Trim any spaces just in case
		headerContentTtype = strings.TrimSpace(headerContentTtype)
		...
		serRec, err := usecases.Unpack(bodyBytes, headerContentTtype)
		if err != nil {
		...
		newRecord, ok := serRec.(*forms.ServiceRecord_v1)
		if !ok {
		...
		if newRecord.Id == 0 {
			...
			// Existing service logic can be added here
		}

		nr, err := usecases.Pack(newRecord, headerContentTtype)
		if err != nil {
			log.Printf("error confirming new service: %s", err)
			http.Error(w, "Error registering service", http.StatusInternalServerError)
		}
		w.Header().Set("Content-Type", headerContentTtype)
		w.WriteHeader(http.StatusOK)
		_, err = w.Write([]byte(nr))
		if err != nil {
...
func (ua *UnitAsset) queryDB(w http.ResponseWriter, r *http.Request) {
	...
	case "GET":
		...
		// text = "</ul></body></html>"
		// w.Write([]byte(text))
		fmt.Fprintf(w, "unsupported http services query method")
	...
	case "POST":
		...
		fmt.Fprintf(w, "unsupported http service discovery method")
	default:
